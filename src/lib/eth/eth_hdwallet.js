...
const generateMasterKey = (mnemonic) => {
...
class EthHdWallet {
   ...
   */
    ...
    /**
     * Generate a new address by hdPath.
     * @return {[String]}
     */
    deriveByPath(hdPath) {
        const child = this._root.derivePath(hdPath).getWallet();;
        const address = normalizeAddress(child.getAddress().toString("hex"))
        return address;
    }

      
    /**
     * get PrivateKey by hdPath.
     * @return {[String]}
     */
    getPrivateKeyByPath(hdPath) {
        const privateKey = this._root
            .derivePath(hdPath)
            .getWallet().getPrivateKey().toString("hex");
        return privateKey;
    }
