...
class EthMonitor extends Monitor {
  constructor({
    ...
    ethSleepTime: sleepTime,
    ethMinimumBlockConfirm: minimumConfirmation,
  }) {
    super({
      balancesHashRepository,
      syncBlockRepository,
    ...
    await Promise.each(
      ...
      async height => {
        ...
        const transactions = await Promise.all([
          this.getEthTransactions(height),
          this.getERC20Transactions(height)
        ]);
  ...
  async getEthTransactions(height) {
  ...
  async getERC20Transactions(height) {
    ...
    const option = {
      ...
      ,
      topics: [constants.ERC20_TOPICS]
    }
    const logTransactions = await this.api.getLogsERC20Address(option);
  ...
  async addMinerInfo(fundings) {
    return await Promise.map(
      ...
      async funding => {
        ...
        console.log("txInfo.gasPrice",txInfo.gasPrice);
        console.log("response.gasUsed",response.gasUsed);
